...
async function _scrape(mirror) {
    ...
    return getContentMatches(mirror.url, /sources:\[(.*?)]/)
        .then((sourcesDiv) => sourcesDiv[1].split('},{')
            .map(sourceDiv => ({
              ...
              resolution: `${sourceDiv.match(/['"]type['"]\s?:\s?['"]([^'"]+)['"]/)[1].toUpperCase()}`
  ...
  } else if (mirror.url.match(/mp4upload\.com\/embed/i) && IS_LOCAL) {
  ...
  } else if (mirror.url.match(/kshows\.to\/load\.php/i) && IS_LOCAL) {
    const parsedUrl = pUrl.parse(mirror.url);
